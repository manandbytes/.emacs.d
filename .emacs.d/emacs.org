...
#+END_SRC
*** TODO Install packages from secure sources
#+BEGIN_SRC emacs-lisp
  (setq el-get-allow-insecure nil)
#+END_SRC
This breaks clean installalation of:
#+BEGIN_SRC
Error (el-get): while installing let-alist: Attempting to install insecure package let-alist without `el-get-allow-insecure'.
Error (el-get): while installing markdown-mode: Attempting to install insecure package markdown-mode without `el-get-allow-insecure'.
Error (el-get): while installing nhexl-mode: Wrong type argument: stringp, nil
Error (el-get): while installing org-mode: Attempting to install insecure package org-mode without `el-get-allow-insecure'.
#+END_SRC

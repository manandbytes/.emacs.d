...
|--------------+---------+-------------------------------------------------------------------------------------------------------------------------------+----------|
| [[https://elpa.gnu.org/packages/][gnu]]          | y       | The [[https://elpa.gnu.org/][GNU Emacs Lisp Package Archive repository]], contains FSF-sanctioned Emacs packages                                         |       90 |
| [[https://elpa.nongnu.org/nongnu/][nongnu]]       | y       | NonGNU ELPA is a [[https://www.gnu.org/philosophy/free-sw.html][free software]] Emacs Lisp Package Archive (ELPA) with extensions to [[https://www.gnu.org/software/emacs/][GNU Emacs]] Archive                         |       70 |
| [[https://melpa.org/packages/][melpa]]        | y       | [[https://melpa.org/packages/][Milkypostman’s Emacs Lisp Package Archive]], built from the latest package source code                                          |       80 |
| [[https://stable.melpa.org/packages/][melpa-stable]] | y       | [[https://melpa.org/packages/][Milkypostman’s Emacs Lisp Package Archive]], built from the latest tagged code in those repositories, where version tags exist. |        0 |

#+begin_src emacs-lisp :var table=package-archives-table :results code :tangle no :exports both
  (let* ((enabled-archives (remove-if (lambda (row) (not (equal (nth 1 row) "y"))) table))
         (archives-as-link (mapcar (lambda (row) (nth 0 row)) enabled-archives))
         (archives-list (mapcar (lambda (archive-as-link)
                                  (with-temp-buffer
                                    ;; https://orgmode.org/worg/dev/org-element-api.html
                                    (insert archive-as-link)
                                    (org-mode)
                                    ...
                                    (let* ((oe-context (org-element-context))
                                      ...
                                      (cons archive-id archive-url))))
                                archives-as-link))
         (archives (sort archives-list (lambda (i1 i2) (string-lessp (car i1) (car i2))))))

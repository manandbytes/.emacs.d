...
** Android clients
- [[github:matburt/mobileorg-android]]
- [[github:wizmer/syncorg]]
- [[github:orgzly/orgzly-android]]
- [[github:kvj/mobileorg-android]]
* TODO What to read
...
- https://wiki.archlinux.org/title/Talk:XDG_Base_Directory

** Integrate Emacs Client

*** Provide .desktop file

#+begin_src conf-desktop :tangle ~/.local/share/applications/emacs-client.desktop
  [Desktop Entry]
  Categories=Office;Development;Utility;TextEditor
  Comment=Connect to an Emacs server using emacsclient
  Encoding=UTF-8
  Exec=emacsclient --create-frame --no-wait --alternate-editor="" %F
  GenericName=Connect to an Emacs server
  Icon=emacs
  Keywords=Text;Editor;
  MimeType=text/*;application/x-shellscript
...
| id                                                   | enabled | description                                                                                                                   | priority |
|------------------------------------------------------+---------+-------------------------------------------------------------------------------------------------------------------------------+----------|
| [[https://elpa.gnu.org/packages/][gnu]]              | y       | The GNU Emacs Lisp Package Archive repository, contains FSF-sanctioned Emacs packages                                         |       90 |
| [[https://elpa.gnu.org/devel/][gnu-devel]]           | y       | GNU-devel ELPA Packages                                                                                                       |          |
| [[https://elpa.nongnu.org/nongnu/][nongnu]]          | y       | NonGNU ELPA is a free software Emacs Lisp Package Archive (ELPA) with extensions to GNU Emacs Archive                         |       70 |
| [[https://melpa.org/packages/][melpa]]               | y       | Milkypostman’s Emacs Lisp Package Archive, built from the latest package source code                                          |       80 |
| [[https://stable.melpa.org/packages/][melpa-stable]] | n       | Milkypostman’s Emacs Lisp Package Archive, built from the latest tagged code in those repositories, where version tags exist. |        0 |

#+begin_src emacs-lisp :var table=package-archives-table :results code :tangle no :exports both
  (let* ((enabled-archives (remove-if (lambda (row)
                                        (not (equal (nth 1 row) "y"))) table))
         (archives-list (mapcar (lambda (row)
                                  (with-temp-buffer
                                    ;; https://orgmode.org/worg/dev/org-element-api.html
                                    (insert (nth 0 row))
                                    (org-mode)
                                    (org-element-contents (org-element-parse-buffer))
                                    (let* ((oe-context (org-element-context))
                                           (archive-id (buffer-substring-no-properties
                                                        (org-element-property :contents-begin oe-context)
                                                        (org-element-property :contents-end oe-context)))
                                           (archive-url (org-element-property :raw-link oe-context))
                                           (archive-priority (if (integerp (nth 3 row)) (nth 3 row) 0)))
                                      (list :id archive-id :url archive-url :priority archive-priority))))
                                enabled-archives))
         (archives (mapcar
                    (lambda (list) (cons (plist-get list :id) (plist-get list :url)))
                    (sort archives-list (lambda (i1 i2)
                                          (string-lessp (plist-get i1 :id) (plist-get i2 :id))))))
         (archives-priorities (mapcar
                               (lambda (list) (cons (plist-get list :id) (plist-get list :priority)))
                               archives-list)))
...
(setq package-archives
	...
	("nongnu" . "https://elpa.nongnu.org/nongnu/"))
      ...
      '(("gnu" . 90) ("gnu-devel" . 0) ("melpa" . 80) ("nongnu" . 70)))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (require 'f)
  (add-to-list 'load-path (f-join user-emacs-directory "el-get" "el-get"))
...
- Emacs package requires native, platform-specific tool. =evernote-mode= requires Ruby interpreter, and it's much simpler to just install =sudo apt-get install evernote-mode= and let it drag in required Ruby implementation and all its transitive dependencies.
...
#+begin_src emacs-lisp :tangle no :results pp :wrap "src emacs-lisp :tangle no"
  (mapcar (lambda (package)
            (let* ((source (el-get-package-def package))
                   (dpkg-package (or (plist-get source :pkgname)
                                     (el-get-as-string package))))
              (list (cons "el-get-package-def" source)
                    (cons "el-get-dpkg-package-installed-p" (el-get-dpkg-package-installed-p dpkg-package))
                    (cons "el-get-read-status-file" (list (cdr (assq (el-get-as-symbol package) (el-get-read-status-file)))))
                    (cons "dpkg-query" (car (process-lines "dpkg-query" "--show" "--showformat=${Status}\n"
                                                                dpkg-package)))
                    (cons "el-get-package-directory" (file-relative-name (el-get-package-directory package) "~/")))))
          (mapcar #'car (el-get-package-types-alist nil 'apt-get)))
#+end_src

#+RESULTS:
#+begin_src emacs-lisp :tangle no
((("el-get-package-def" :name csv-mode :type apt-get :pkgname "elpa-csv-mode" :after nil)
  ("el-get-dpkg-package-installed-p" . t)
  ("el-get-read-status-file"
   (status "installed" recipe
	   (:name csv-mode :type apt-get :pkgname "elpa-csv-mode" :after nil)))
  ("dpkg-query" . "install ok installed")
  ("el-get-package-directory" . ".config/emacs/el-get/csv-mode/"))
 (("el-get-package-def" :name org-contrib :type apt-get :pkgname "elpa-org-contrib" :after nil :depends
   (org-mode))
  ("el-get-dpkg-package-installed-p" . t)
  ("el-get-read-status-file"
   (status "installed" recipe
	   (:name org-contrib :type apt-get :pkgname "elpa-org-contrib" :after nil :depends
		  (org-mode))))
  ("dpkg-query" . "install ok installed")
  ("el-get-package-directory" . ".config/emacs/el-get/org-contrib/")))
#+end_src
...
=el-get= doesn't support installation of platform-specific packages.
*** System Packages

#+begin_src emacs-lisp
  (req-package system-packages
    :ensure)
  (req-package use-package-ensure-system-package
    :ensure)
#+end_src
...
#+begin_src emacs-lisp :tangle no :exports both :results pp :wrap "src emacs-lisp :tangle no"
  (let* ((required-dpkg-packages (el-get-package-types-alist nil 'apt-get))
         (packages (delete-dups (mapcar 'car required-dpkg-packages))))
    (el-get-read-status-file-force)
    (dolist (package packages)
      (let* ((pkg-el-dir (directory-file-name (el-get-package-directory package)))
             (apt-get-pkgname (plist-get (el-get-package-def package) :pkgname))
        ...
        ;; el-get dir apt-get package must be a symlink to real packages dir
        (el-get-dpkg-remove-symlink package)
        (when (and (file-directory-p pkg-el-dir)
                   (not (file-symlink-p pkg-el-dir)))
          (delete-directory pkg-el-dir t))
        (make-symbolic-link (first pkg-el-dirs) pkg-el-dir)
...
#+RESULTS:
#+begin_src emacs-lisp :tangle no
(("/usr/share/emacs/site-lisp/elpa-src/csv-mode-1.23/")
 ("/usr/share/emacs/site-lisp/elpa-src/org-contrib-0.4.2/"))
#+end_src

** TODO How to remove unnecessary packages
...
The simplest solution is to blindly remove every package which is not explicitly referenced by =el-get-sources=:
...
** Browsing
Debian ships two flavors of Mozilla Firefox: [[deb:firefox][Firefox for desktop]] and [[deb:firefox-esr][Firefox Extended Support Release (ESR)]]. See [[https://www.mozilla.org/en-US/firefox/organizations/faq/][Who should use Mozilla Firefox ESR?]] for more info.
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC sh :dir "~/.config/emacs/el-get/dash" :results format org
...
#+BEGIN_SRC emacs-lisp :results pp :tangle no :dir ~
  (split-string (shell-command-to-string "vcsh list-tracked bash"))
#+END_SRC

#+RESULTS:
: ("/home/mn/.bash_aliases" "/home/mn/.bash_logout" "/home/mn/.bashrc" "/home/mn/.config/byobu/status" "/home/mn/.config/byobu/statusrc" "/home/mn/.config/byobu/.tmux.conf" "/home/mn/.config/gnome-session/sessions/gnome-i3.desktop" "/home/mn/.config/gnome-session/sessions/gnome-i3.session" "/home/mn/.config/i3/config" "/home/mn/.config/i3status/config" "/home/mn/.config/liquidpromptrc" "/home/mn/.config/rofi/config" "/home/mn/.config/tmux/inx" "/home/mn/.config/tmux/xless" "/home/mn/.profile" "/home/mn/.ssh/config" "/home/mn/.tmux.conf")
** Resolving conflicts
...
** TODO Integrate Magit and Gerrit Code Review
...
** Disable bars
#+BEGIN_SRC emacs-lisp
  (dolist (mode-to-disable '(menu-bar-mode tool-bar-mode scroll-bar-mode))
    (when (functionp mode-to-disable)
...
#+BEGIN_SRC sh :dir ~/.config/emacs/el-get/org-mode :tangle no :exports results
...
*** TODO Emacs built-in project support
#+begin_src emacs-lisp
  (el-get-bundle project :builtin "29")
#+end_src
...
#+begin_src emacs-lisp
  ...
  (req-package projectile
    ...
    :custom ((projectile-mode-line-prefix " P")
             (projectile-switch-project-action #'projectile-commander "Show commander when switching project"))
    :config
    (projectile-mode))
...
*** Gitlab
#+BEGIN_SRC emacs-lisp
  (add-to-list 'org-link-abbrev-alist '("gitlab" . "https://gitlab.com/%s"))
#+END_SRC
...
- use =org-babel-demarcate-block= (bound to =C-c C-v d= by default) outside of a code block
- type =<s= followed by =Tab=
*** Split source code block
...
#+BEGIN_SRC emacs-lisp
  (el-get-bundle org-link-travis
    ...
    :depends (org-mode))
#+END_SRC
** TODO Switch from 'org-add-link-type to 'org-link-set-parameters
